...
class Blueprint_UI:
    ...
    def initialiseModuleTab(self, tabHeight, tabWidth):
        ...
        scrollHeight = tabHeight -moduleSpecific_scrollHeight - 163#v046
    ...
    def initializeTemplatesTab(self, tabHeight, tabWidth):
        ...
        for template in utils.findAllMayaFiles("/Templates"):
            ...
            templateAndPath = os.environ["GEPPETTO"] + "/Templates/" + template + ".ma"
    ...
    def publish(self, *args):
        ...
        result = cmds.promptDialog(title="Publish Character", message="Please specify a character name ([a-z][0-9] and_only):", button=["Accept", "Cancel"], defaultButton="Accept", cancelButton="Cancel", dismissString="Cancel")
        if result == "Accept":
            ...
            projectDirs = turbineUtils.setupDirs(self, characterName, create=True)
            ...
            gameJoints = cmds.ls(sl=True)

            cmds.container("Trash", edit=True, addNode=gameJoints, force=True)
               
            for container in characterContainers:
                cmds.lockNode(container, lock=True, lockUnpublished=True)
            
            # Export an xml file containing game joint info.  This will be replaced by a dataBase.    
            xmlFileName = xmlDir + characterName + "_xml" + ".xml"
            utils.exportJointInfo(xmlFileName)
